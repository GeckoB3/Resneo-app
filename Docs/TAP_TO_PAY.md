...
### Stuck-prepare / stuck-pending pass (ninth, 2026-07-29)
Two defects reported from a device, both about the app waiting for something that was never going to arrive.

- **"Getting the card reader ready" span for ever, and Back was dead.** Root cause is a semantic in the pinned SDK, not our state machine: for `bluetoothScan`, `discoverReaders`' promise is resolved from the discovery **completion** handler (iOS `discoverCancelable`'s completion, Android `RNDiscoveryListener`) and the discovery configuration's timeout defaults to `0` = *never*. A Bluetooth scan therefore ends only when it is cancelled or when a successful `connectReader` ends it. `scan()` awaited that promise as if it meant "discovery finished", so `reconnectRemembered()` — which awaits the scan, then connects — waited for a discovery that only the connect it was gating could end. A **deadlock**, and because `busy` disabled every button including Back, staff were stuck until they closed the whole sheet. (Closing worked because the unmount cleanup cancels discovery, which ends the session.) Now: the scan waits on the readers callback / the session ending / a 20s budget, whichever comes first (`lib/payments/reader-timeouts.ts` holds the budgets and the reasoning), every other await in both reader paths is time-boxed, the live discovery session is tracked **module-level** so the sheet's two `useBluetoothReader` instances can hand over without the SDK answering "busy", and the bottom button reads **Cancel** during a prepare and aborts the attempt cleanly.
- **A declined tap kept warning "a card payment is still going through".** See backend requirement 4: the row is `pending` until the webhook says otherwise. The app now records attempts it watched fail (`lib/payments/failed-attempts.ts`, gated by `isDefiniteCardFailure` so an *ambiguous* confirm still warns), discounts them from the gate and the notices, invalidates caches on card-mutation **error** as well as success, and — for a row nothing can explain — softens the hard gate to an informational notice past `PENDING_CARD_STALE_MS` (the same window the settlement watch gives up at) telling staff to check the payment history or the Stripe dashboard.

### Backend requirements
Five things this pass could **not** fix app-side. All live in `src/app/api/venue/bookings/[id]/charge/route.ts` unless stated.
...
4. **The booking GET hides `stripe_payment_intent_id`, so a ledger row cannot be identified.** `src/app/api/venue/bookings/[id]/route.ts` selects `id, booking_id, method, status, amount_pence, note, created_at` from `booking_payments` — no PaymentIntent id and no `metadata` (which holds the client's `attempt_id`'s siblings). The app therefore cannot join the row to the PaymentIntent it just drove, even though the charge route handed it `payment_intent_id` seconds earlier.
   Why that matters: the row is written `pending` at PI-create time and ONLY the webhook moves it (`payment_intent.payment_failed` → `failed`). When a tap is **declined**, the app knows for certain the money did not move, but the ledger still reads "in flight" until the failed webhook lands — in local dev, possibly never — so the sheet warned staff "a card payment is still going through" for a payment they had watched be refused, and blocked them from collecting again (found on device).
   **App-side mitigation that shipped:** `lib/payments/failed-attempts.ts` remembers attempts this client watched fail and matches them to rows by *booking + amount + a ±2-minute created_at window*, which is the best identity available. It is deliberately conservative — any mismatch keeps warning — but it can be defeated by a colleague collecting the *same amount* on the *same booking* inside that window.
