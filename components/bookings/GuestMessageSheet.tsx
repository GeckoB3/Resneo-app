...
import { useSendBookingMessage, type GuestMessageChannel } from '@/lib/queries/useBookingMutations';
...
function channelOptions(
  email?: string | null,
  phone?: string | null,
): { value: GuestMessageChannel; label: string }[] {
  const hasEmail = !!email?.trim();
  const hasPhone = !!phone?.trim();
  const options: { value: GuestMessageChannel; label: string }[] = [];
  if (hasEmail) options.push({ value: 'email', label: 'Email' });
  if (hasPhone) options.push({ value: 'sms', label: 'SMS' });
  if (hasEmail && hasPhone) options.push({ value: 'both', label: 'Both' });
  return options;
}

/** Bottom-sheet to send a custom email/SMS message to the booking's guest. */
export function GuestMessageSheet({ target, onClose }: GuestMessageSheetProps) {
  const { colors } = useTheme();
  const mutation = useSendBookingMessage(target?.id ?? '');
  ...
  // Seed defaults when a new target opens.
