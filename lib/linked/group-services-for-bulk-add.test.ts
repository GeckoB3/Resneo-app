import { groupServicesForBulkAdd } from '@/lib/linked/group-services-for-bulk-add';

/**
 * Bulk-add grouping for the combined-page manager. Ported from the web
 * `group-services-for-bulk-add.test.ts` (vitest -> jest) so the merge rules
...
describe('groupServicesForBulkAdd', () => {
  it('keeps distinctly named services as separate offerings', () => {
    ...
    expect(groups[0]).toEqual({
      name: 'Massage',
      sources: [{ venueId: 'v1', sourceServiceId: 's1' }],
    });
    expect(groups[1]).toEqual({
      name: 'Facial',
      sources: [{ venueId: 'v1', sourceServiceId: 's2' }],
    });
  });
