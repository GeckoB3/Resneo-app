...
/**
 * Venue-level compliance general settings — the app-side port of the web
 * `src/lib/compliance/config.ts` (single source of truth). Persisted in
 * `venues.feature_flags.compliance` (a nested object alongside the boolean
 * `compliance_records_enabled` flag). All fields carry sensible defaults so an
 * absent/partial object resolves to a complete config.
 *
 * NOTE: the retired venue-wide `auto_send_on_booking` toggle is deliberately
 * absent (the web schema `.strip()`s it); that behaviour moved to the
 * per-requirement `online_collection` field. `reminder_cadence_days` defaults
 * to 0 (no reminder) to match the web.
 */
...
/** v1 only supports `warn_only`; `block_check_in` is deferred to v2. */
...
/** Platform default form-link expiry when neither type nor venue overrides it. */
...
export const complianceConfigSchema = z
  .object({
    ...
    /** Hours before a booking the client must complete required forms (default for new requirements). */
 ...
 * per-type override, then venue general setting, then platform default (14).
 */
