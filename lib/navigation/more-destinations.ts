 ...
 * grid / grouped-list / search all derive from one source AND the gating can be
...
export type DestGroup = 'workspace' | 'manage' | 'bookingTypes' | 'app' | 'account';
 ...
 * gating can be unit-tested without rendering.
 */
export function buildDestinations(ctx: DestinationsContext): Destination[] {
  ...
  list.push({ id: 'today', label: 'Today', hint: 'KPIs, forecast & arrivals', icon: { ios: 'sun.max.fill', android: 'wb_sunny', web: 'wb_sunny' }, tile: TILE.amber, group: 'workspace', kind: 'route', target: '/today', featured: true, keywords: ['home', 'dashboard', 'overview'] });
  if (isAdmin) {
  ...
  // Calendar availability — scheduling-experience venues only (web gates this on
  // appointment availability eligibility, not role).
  ...
  list.push({ id: 'services', label: 'Services', hint: 'Review & edit your appointment services', icon: { ios: 'scissors', android: 'content_cut', web: 'content_cut' }, tile: TILE.navy, group: 'manage', kind: 'route', target: '/manage/services', keywords: ['treatments', 'pricing', 'duration'] });
  if (isAdmin) {
    list.push({ id: 'venue-profile', label: 'Venue profile', hint: 'Name, contact details & address', icon: { ios: 'building.2.fill', android: 'storefront', web: 'storefront' }, tile: TILE.teal, group: 'manage', kind: 'route', target: '/manage/venue-profile', keywords: ['settings', 'business', 'location'] });
  }
  list.push({ id: 'hours', label: 'Business hours', hint: 'Weekly opening hours', icon: { ios: 'clock.fill', android: 'access_time', web: 'access_time' }, tile: TILE.sky, group: 'manage', kind: 'route', target: '/manage/hours', keywords: ['settings', 'opening', 'closures', 'exceptions'] });
  list.push({ id: 'linked-calendar', label: 'Linked calendar', hint: 'View bookings at venues linked to yours', icon: { ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }, tile: TILE.teal, group: 'manage', kind: 'route', target: '/linked-venues/calendar', keywords: ['linked accounts', 'cross-venue'] });
  // Compliance — appointment tier + records flag, shown to staff AND admin (web parity).
  if (isAppointmentTier && complianceEnabled) {
    list.push({ id: 'compliance', label: 'Compliance', hint: 'Forms, records & expiries', icon: { ios: 'checkmark.shield.fill', android: 'verified_user', web: 'verified_user' }, tile: TILE.emerald, group: 'manage', kind: 'route', target: '/manage/compliance', keywords: ['consent', 'forms', 'documents', 'certificates'] });
  }
  if (isAdmin) {
    list.push({ id: 'team', label: 'Team', hint: 'Staff logins & roles', icon: { ios: 'person.2.fill', android: 'group', web: 'group' }, tile: TILE.emerald, group: 'manage', kind: 'route', target: '/manage/team', keywords: ['settings', 'staff', 'roles', 'permissions'] });
    list.push({ id: 'booking-settings', label: 'Booking settings', hint: 'Booking types & guest accounts', icon: { ios: 'slider.horizontal.3', android: 'tune', web: 'tune' }, tile: TILE.slate, group: 'manage', kind: 'route', target: '/manage/booking-settings', keywords: ['settings', 'guests', 'waitlist', 'deposits'] });
    list.push({ id: 'communications', label: 'Communications', hint: 'Confirmations, reminders & alerts', icon: { ios: 'envelope.fill', android: 'mail', web: 'mail' }, tile: TILE.amber, group: 'manage', kind: 'route', target: '/manage/communications', keywords: ['settings', 'SMS', 'email', 'templates', 'reminders', 'notifications'] });
    list.push({ id: 'plan', label: 'Plan & payments', hint: 'Subscription tier & Stripe', icon: { ios: 'creditcard.fill', android: 'credit_card', web: 'credit_card' }, tile: TILE.violet, group: 'manage', kind: 'route', target: '/manage/plan', keywords: ['settings', 'payments', 'stripe', 'billing', 'subscription', 'invoice'] });
    list.push({ id: 'refer-earn', label: 'Refer & Earn', hint: 'Share your link & track rewards', icon: { ios: 'gift.fill', android: 'card_giftcard', web: 'card_giftcard' }, tile: TILE.rose, group: 'manage', kind: 'route', target: '/manage/refer-earn', keywords: ['referral', 'rewards', 'credit', 'invite'] });
    list.push({ id: 'booking-page', label: 'Booking page', hint: 'Branding, colours, fonts & public tabs', icon: { ios: 'globe', android: 'public', web: 'public' }, tile: TILE.indigo, group: 'manage', kind: 'route', target: '/manage/booking-page', keywords: ['settings', 'branding', 'public', 'embed', 'widget'] });
    list.push({ id: 'import-contacts', label: 'Import contacts', hint: 'Upload a CSV of clients on the web', icon: { ios: 'square.and.arrow.down', android: 'upload_file', web: 'upload_file' }, tile: TILE.sky, group: 'manage', kind: 'web', target: '/dashboard/import', external: true, keywords: ['csv', 'upload', 'bulk', 'clients', 'guests', 'bookings', 'data'] });
    list.push({ id: 'linked-venues', label: 'Linked venues', hint: 'Share calendars & cross-venue bookings', icon: { ios: 'link', android: 'link', web: 'link' }, tile: TILE.teal, group: 'manage', kind: 'route', target: '/linked-venues', keywords: ['settings', 'linked accounts', 'partners'] });
    list.push({ id: 'collectives', label: 'Venue collectives', hint: 'A combined booking page across linked venues', icon: { ios: 'person.2.wave.2.fill', android: 'groups', web: 'groups' }, tile: TILE.sky, group: 'manage', kind: 'route', target: '/collectives', keywords: ['combined', 'linked venues'] });
  }

  // Booking types — only the models this venue has enabled. Model-driven, NOT
  // role-gated (web parity): staff at a class/event/resource venue reach them too.
